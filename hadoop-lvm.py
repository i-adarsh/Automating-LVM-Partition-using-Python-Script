...
def decrease ():
	...
	os.system(f"resize2fs /dev/hadoop/hadoop-dn {want}G")
	os.system(f"lvreduce --size -{present - want}G /dev/hadoop/hadoop-dn -f")
...
def config ():
	...
	os.system("umount /hadoop-lvm")	
	os.system("rm -rf /hadoop-lvm")	
	os.system("mkdir /hadoop-lvm")	
	os.system(f"mkfs.ext4 /dev/hadoop/hadoop-dn")
	os.system(f"mount /dev/hadoop/hadoop-dn  /hadoop-lvm")
	os.system("hadoop-daemon.sh start datanode")
	os.system("jps")
